<?php 
	...
	if(isset($_POST['submit']) && $_POST['submit'] == 'Update'){
		...
		// make sure the username and user_id is a valid pair ( we don't want people to try and manipulate the form tpo hack someone else's account)
		
		$query = 'SELECT username FROM site_user WHERE user_id = "' . (int)$user_id . '" AND username = "' . mysqli_real_escape_string($_SESSION['username'], $db) . '"';
 ...
 <?php 
 	...
 	if(count($errors) > 0){
 		...
 		$query = 'UPDATE site_user_info SET 
 				...
 				city = "' . mysqli_real_escape_string($city, $db) . '",
 				state = "' . mysqli_real_escape_string($state, $db) . '",
 				hobbies = "' . mysqli_real_escape_string(join(',', $hobbies), $db) . '"
